...
describe("telegram heartbeat helpers", () => {
  test("defaults heartbeat conversations to gpt-5-mini when TELEGRAM_BOT_MODEL is unset", () => {
  ...
  test("falls back heartbeat conversations to gpt-5-mini for unsupported TELEGRAM_BOT_MODEL", () => {
    const logMessages: string[] = [];
    ...
    expect(logMessages).toEqual([
      'Unsupported TELEGRAM_BOT_MODEL "not-a-real-model", falling back to gpt-5-mini',
    ]);
  });
