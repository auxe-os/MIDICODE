...
*   `bun run dev:vite` is enough to verify the window, screen, themes, and click-wheel interface locally.
*   `bun run dev` is recommended when you want the API-backed library flows to behave like the full app.
*   Features that depend on shared song metadata, syncing, or server-backed media flows are most reliable with Redis configured and the standalone API running.
...
The wheel is available across the classic, black, and U2 themes. In the classic theme, the wheel intentionally uses higher-contrast shading so the navigation ring and center button remain readable against the white body.
...
*   Redis-backed song metadata powers the shared library and update checks.
*   Imported YouTube-based media flows rely on API routes and cached metadata.
*   Shared-library behavior overlaps with Karaoke and other media features through the same store and backend data sources.

## Related Apps

The iPod app integrates well with ryOS's system-wide [audio controls](/docs/audio-system) for volume management. Users might also find the [TextEdit](/docs/textedit) app useful for jotting down song ideas or creating custom playlists for later use.
