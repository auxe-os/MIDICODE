...
Internet Explorer depends on the local API even for standard browsing flows. Current-year navigation and embeddability checks go through the `/api/iframe-check` backend route, so `bun run dev:vite` by itself is not enough for full local IE testing.

#### Recommended local modes
*   **Frontend-only UI smoke test:** `bun run dev:vite`
    *   Useful for opening the desktop and checking basic app chrome.
    *   Internet Explorer navigation will be limited because `/api/*` routes are unavailable.
*   **Full local IE testing:** `bun run dev`
    *   Starts both the standalone Bun API and the Vite frontend.
    *   Requires Redis configuration in `.env.local`.

#### Time-travel generation requirements
*   **Redis** for caching and generated timeline support
*   **AI provider key** such as `OPENAI_API_KEY` for generated pre-1996 and future pages
...
*   `/api/iframe-check` handles embeddability checks, proxying, and local navigation safety.
*   `/api/ie-generate` powers AI-assisted historical and speculative page generation.
*   Redis-backed caching improves repeated timeline requests and generated-page retrieval.
