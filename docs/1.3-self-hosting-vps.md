...
Coolify + Bun is the recommended production path for this repo. Vercel-compatible code paths remain supported, but they are no longer the default deployment recommendation.
...
Before you start, create your local env file from the tracked template:

```bash
cp .env.example .env.local
```

`.env.local` is local-only and ignored by git. Never commit real secrets. If a secret is pasted into chat, an issue, or a commit, rotate it immediately.
...
```bash
cp .env.example .env.local
# set REDIS_KV_REST_API_URL and REDIS_KV_REST_API_TOKEN
bun run dev
```
...
```bash
cp .env.example .env.local
# set REDIS_URL, REALTIME_PROVIDER=local, REALTIME_WS_PATH=/ws
bun run dev
```
...
S3_REGION=us-east-1
S3_ENDPOINT=https://s3.your-provider.com
S3_PUBLIC_ENDPOINT=https://s3.your-provider.com
S3_ACCESS_KEY_ID=your-access-key
S3_SECRET_ACCESS_KEY=your-secret-key
```
...
- keep WebSocket proxying enabled for `/ws`
- prefer `REDIS_URL`, `REALTIME_PROVIDER=local`, and `STORAGE_PROVIDER=s3`
- use `API_ALLOWED_ORIGINS` to match the real browser origins
