...
|--------|------------|-------------|
| **Web (PWA)** | Coolify / Docker / Bun | Recommended production deployment using the Bun server behind a container or reverse proxy |
...
| **Standalone API** | Bun (`Bun.serve`) | Run `api/*` routes directly via `scripts/api-standalone-server.ts` |
| **Vercel-compatible** | Vercel | Supported compatibility path for serverless deployment and platform integrations |
...
- **Standalone Bun server:** `scripts/api-standalone-server.ts` uses `Bun.serve` and serves the frontend, API routes, docs, health checks, and optional local WebSocket realtime.
- **Vercel serverless functions:** `api/*` handlers remain compatible with Vercel's Node.js runtime for teams that still want that deployment mode.
